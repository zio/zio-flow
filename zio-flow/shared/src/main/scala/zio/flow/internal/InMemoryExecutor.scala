...
final case class InMemoryExecutor[U, R <: Clock: Tag](
  ...
  def compile[I, E, A](
    ...
    flow match {
      case Return(value) =>
        value.evalDynamic.intoPromise(promise) as CompileStatus.Done
      ...
      case WaitTill(instant) =>
        (for {
          start <- clock.instant
          end   <- instant.eval(schemaInstant)
      ...
      case Modify(svar, f0) =>
        ...
        val f = f0
        (for {
          variable <- svar.evalDynamic
          optValue <- RemoteContext.getVariable(variable.identifier)
          value <- ZIO
                     .fromOption(optValue)
                     .orElse(ZIO.fail(s"Undefined variable ${variable.identifier} in Modify"))
          tuple             <- f(lit(value)).eval(flow.resultSchema.schema)
