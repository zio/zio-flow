...
//}
//import zio.schema._
//import zio.{Clock, IO, Promise, Ref, Tag, UIO, ZEnvironment, ZIO}
//
...
// */
//
//final case class InMemoryExecutor[U, R <: Clock: Tag](
//  env: ZEnvironment[R],
//  execEnv: ExecutionEnvironment,
//  opExec: OperationExecutor[R],
//  workflows: Ref[Map[U, Ref[InMemoryExecutor.State]]]
//) extends ZFlowExecutor[U] {
//
//  import InMemoryExecutor._
//  import ZFlow._
//
...
//             )
//             .orDieWith(s => new RuntimeException(s))
...
//    input: I,
//    flow: ZFlow[I, E, A]
//  ): ZIO[R with RemoteContext, String, CompileStatus] =
//    flow match {
//      case Return(value) =>
//        for {
//          value <- value.eval[A](Schema.fail[A]("no schema"))
...
//        for {
//          start <- clock.instant
//          end   <- instant.eval(zio.flow.instantSchema)
//          _     <- clock.sleep(Duration.between(start, end))
//          _     <- promise.asInstanceOf[Promise[Nothing, Unit]].succeed(())
//        } yield CompileStatus.Done
//
//      case Modify(svar, f0) =>
...
//        val f = f0
//        (for {
//          variable <- svar.eval[Remote.Variable[Any]]
//          optValue <- RemoteContext.getVariable(variable.identifier)
//          value <- ZIO
//                     .fromOption(optValue)
//                     .orElseFail(s"Undefined variable ${variable.identifier} in Modify")
//          tuple             <- f(lit(value)).eval
...
//      case Unwrap(remote) =>
//        for {
//          evaluatedFlow <- remote.eval
//          status        <- compile(promise, ref, input, evaluatedFlow)
//        } yield status
//
//      case UnwrapRemote(remote) =>
//        for {
//          evaluatedRemote <- remote.eval[Remote[Any]]
//          evaluated       <- evaluatedRemote.eval(flow.resultSchema)
...
//          status <-
//            provide.value.eval
//              .flatMap(valueA => compile(innerPromise, ref, valueA, provide.flow)) //TODO : Input is provided in compile
...
//        (for {
//          value <- eval(initial)
//          vref   = Remote.Variable(RemoteVariableName(name))
...
//  ): UIO[InMemoryExecutor[U, R]] =
//    (for {
//      ref    <- Ref.make[Map[U, Ref[InMemoryExecutor.State]]](Map.empty)
//      execEnv = ExecutionEnvironment(serializer, deserializer)
//    } yield InMemoryExecutor[U, R](env, execEnv, opEx, ref))
//
