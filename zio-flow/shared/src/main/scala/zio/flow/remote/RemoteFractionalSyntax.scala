...
class RemoteFractionalSyntax[A](self: Remote[A]) {
  ...
  final def sin(implicit fractional: remote.Fractional[A]): Remote[A] =
    Remote.SinFractional(self, fractional)

  final def cos(implicit fractional: remote.Fractional[A]): Remote[A] = {
  ...
  final def tan(implicit fractional: remote.Fractional[A]): Remote[A] =
    sin(fractional) / cos(fractional)

  final def sinInverse(implicit fractional: remote.Fractional[A]): Remote[A] =
    Remote.SinInverseFractional(self, fractional)

  final def cosInverse(implicit fractional: remote.Fractional[A]): Remote[A] = {
    implicit val schema = fractional.schema
    Remote(fractional.fromDouble(1.571)) - sinInverse(fractional)
  }

  final def tanInverse(implicit fractional: remote.Fractional[A]): Remote[A] =
