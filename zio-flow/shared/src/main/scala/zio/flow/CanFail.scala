...
object CanFail extends CanFail[Any] {
  ...
  /**
   * Providing multiple ambiguous values for CanFail[Nothing] makes it not
   * compile when `E = Nothing`. We need this because `CanFail`` should always
   * have a legitimate error type `E` representing a ZFlow that can fail with
   * that `error`.
   */
