...
import zio.test.Assertion.{dies, equalTo, fails, hasMessage, nothing}
...
object ZFlowExecutorSpec extends DefaultRunnableSpec {
  ...
  implicit def variableSchema[A]: Schema[Variable[A]] = ???
  ...
  val suite1: Spec[Environment, TestFailure[Any], TestSuccess] = suite("Test Unwrap")(
    testM("Test Return") {
      val compileResult = ZFlow.Return(12).evaluateTestInMem(implicitly[Schema[Int]], nothingSchema)
    ...
    testM("Test Unwrap") {
      val compileResult = ZFlow.Unwrap[Any, Nothing, Int](Remote(ZFlow.succeed(12))).evaluateTestInMem(implicitly[Schema[Int]], nothingSchema)
      assertM(compileResult)(equalTo(12))
    },
    testM("Test ForEach") {
      val compileResult = ZFlow.Foreach[Any, Nothing, Int, Int](Remote(List(10, 20, 30)), (a: Remote[Int]) => a + 5).evaluateTestInMem(implicitly[Schema[List[Int]]], nothingSchema)
      assertM(compileResult)(equalTo(List(15, 25, 35)))
    },
    testM("Test Ensuring") {
      val compileResult = ZFlow.Ensuring(ZFlow.succeed(12), ZFlow.input[String]).provide("Some input").evaluateTestInMem(implicitly[Schema[Int]], nothingSchema)
      assertM(compileResult)(equalTo(12))
    },
    testM("Test Transaction") {
      val compileResult = ZFlow.Transaction[Any, Nothing, Int](ZFlow.succeed(12)).evaluateTestInMem(implicitly[Schema[Int]], nothingSchema)
      assertM(compileResult)(equalTo(12))
    },
    testM("Test Input and Provide"){
      val compileResult =  ZFlow.input[String].provide("Some input").evaluateTestInMem(implicitly[Schema[String]], nothingSchema)
      assertM(compileResult)(equalTo("Some input"))
    },
    testM("Test Fork") {
      def executingFlowSchema[E, A]: Schema[ExecutingFlow[E, A]] = ???
    ...
    testM("Test Die")(
      assertM(ZFlow.Die.evaluateTestInMem(nothingSchema, nothingSchema).run)(dies(hasMessage(equalTo("Could not evaluate ZFlow"))))
    ),
    testM("Test RunActivity") {
      implicit val activityErrorSchema : Schema[ActivityError] = ???
      val compileResult = ZFlow.RunActivity(12, testActivity).evaluateTestInMem(implicitly[Schema[Int]], activityErrorSchema)
      assertM(compileResult)(equalTo(12))
    },
    testM("Test NewVar")(
      ZFlow.NewVar("var1", Remote(20)).evaluateTestInMem(variableSchema[Int], nothingSchema).map(r => assert(r.isInstanceOf[Variable[Int]])(equalTo(true)))
    ),
    testM("Retry Until - No Transaction") {
      assertM(ZFlow.RetryUntil.evaluateTestInMem(nothingSchema, nothingSchema).run)(dies(hasMessage(equalTo("There is no transaction to retry."))))
    },
    testM("Test OrTry - right succeeds") {
      val compileResult = ZFlow.OrTry[Any, Nothing, Int](ZFlow.transaction(_ => ZFlow.RetryUntil), ZFlow.succeed(12)).evaluateTestInMem(implicitly[Schema[Int]], nothingSchema)
      assertM(compileResult)(equalTo(12))
    },
    testM("Test OrTry - left succeeds") {
      val compileResult =  ZFlow.OrTry[Any, Nothing, Int](ZFlow.succeed(12), ZFlow.transaction(_ => ZFlow.RetryUntil)).evaluateTestInMem(implicitly[Schema[Int]], nothingSchema)
      assertM(compileResult)(equalTo(12))
    },
    testM("Test Fold") {
      checkM(Gen.anyString, Gen.anyInt) { case (s, v) =>
        val compileResult1 = ZFlow.succeed(s).foldM(ifError, ifSuccess).evaluateTestInMem(implicitly[Schema[Int]], nothingSchema)
        assertM(compileResult1)(equalTo(s.length))
        val compileResult2 = ZFlow.fail(v).foldM(ifError, ifSuccess).evaluateTestInMem(implicitly[Schema[Int]], nothingSchema)
    ...
    testM("Test Iterate") {
      val compileResult = ZFlow.Iterate[Any, ActivityError, Int](
